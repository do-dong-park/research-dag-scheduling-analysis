...
from custom_sched import sched, trace_init
    ...
    def run(self):
        ...
        for task_idx in tqdm(range(self.taskset_size)):
            ...
            random_seeds = random.sample(
                range(self.randomization_times * 100), self.randomization_times
            )

            # load the DAG task
            dag_task_file = "./data/data-generic/Tau_{:d}.gpickle".format(task_idx)
            ...
            for algorithm_name in ("random", "eligibility"):
                ...
                for m in (2, 4):
                    ...
                    for e_model in ("WCET", "full_random", "half_random"):
                        makespans = []
                        makespan = -1
                        if algorithm_name == "random" or e_model != "WCET":
                        ...
                        message = "{:d};{:s};{:d};{:s};{:s}".format(
                            task_idx,
                            algorithm_name,
                            m,
                            e_model,
                            str(makespans).replace(" ", ""),
                        )
                        print(message)
